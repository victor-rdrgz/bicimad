...
class BiciMad():
  ...
  def fleet_1_bikes(self) -> pd.DataFrame:
    ...
    return self.data[self.data['fleet'] == '1'].reset_index(drop=True)


  def day_time(self) -> pd.Series:
    '''C3: Calculates how many hours have all the bikes been
    rented divided per day'''
    rent_by_day = round(self.data.groupby('fecha')['trip_minutes'].sum()/60, 2)
    rent_by_day.index = rent_by_day.index.date

    return rent_by_day


  def daily_rents_bar_diagram_from_df(self) -> None:
  ...
  @staticmethod
  def bar_diagram_from_serie(serie: pd.Series) -> None:
    '''C3: Create a chart with the data from a series that
    contains the number of daily rentals for a month.'''
    ax = serie.plot(
        kind='bar',
  ...
  def weekday_time(self) -> pd.Series:
    ...
    total_hours = (
      self.data['trip_minutes'].groupby(level=0).sum() / 60)
    total_hours.index = pd.to_datetime(total_hours.index)
    result = total_hours.groupby(total_hours.index.day_name()).sum()
    day_order = [
        'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 
        'Saturday', 'Sunday']
    result = result.reindex(day_order).round(2)
    result.name = 'Trip_hours'
    return result
